...
(function() {
  var HrArticle, anchor, article, articleList, clickTag, evalFilterFunction, filterByFunction, filterByNumberList, j, len, li, list, outputMatchNumber, parseQueryString, queryForm, queryObject, ref, theTag;
  ...
  if (loadArticle) {
    articleList = [];
    loadArticle.then(function(list) {
      return articleList.push.apply(articleList, list);
    });
  } else {
  ...
  clickTag = function(evt) {
    evt.preventDefault();
    queryObject = parseQueryString(this.href.replace(/^.*\?/, ''));
    theTag = queryObject.tags;
    list = filterByFunction(function(article) {
      return article.hasTag(theTag);
    });
    return history.pushState({
      list: list
    }, "tag: " + queryObject, this.href);
  };

  ref = document.getElementsByTagName('li');
  for (j = 0, len = ref.length; j < len; j++) {
    li = ref[j];
